...
| **✅ README** | The licence term was read in the repository's own `README` or on the repo page, because the project ships **no** `LICENSE`/`COPYING` file — or ships one that does not carry the term in question. Weaker than `✅ file`: a README statement is the authors' assertion, not an executed licence text, so it is recorded but never used to justify a dependency |
...
Every `⚠️ UNVERIFIED` row that gates a dependency, a default, a data or taxonomy decision, or a provider we intend to use is carried in §12's blocking queue. The rest — closed products we will never integrate, research-gated datasets policy already excludes, and specification repositories whose licence never reaches our code — gate nothing that ships and are recorded here and in [`research/`](./research/) only.
...
| ComfyUI-KikoTools | `ComfyAssets/ComfyUI-KikoTools` | MIT | ✅ label | README (Gemini Prompt Engineer, Local Image Loader) | Concepts only, no code: analyse once, format many — one analysis pass, several output dialects chosen by a parameter; a passthrough inspector as precedent for a `visual_intent` debug view | The Gemini binding: a hardcoded, cloud-only, single-vendor analysis path — precisely what the brief prohibits. API-key handling. Any source |
...
| IDM-VTON | `yisol/IDM-VTON` | **CC BY-NC-SA-4.0** (code **and** checkpoints) | ✅ README | README, licence statement, input contract | Concepts only: its input contract is decomposition by construction — person signals (pose, body region) separated from garment signals. Our analyzer should likewise emit `pose`/`appearance` separately from `clothing` | **Nothing shippable.** NC blocks commercial use of both code and checkpoints; SA would infect derivatives. Additionally trained on VITON-HD / Dress Code, a second independent block |
...
| CatVTON | `Zheng-Chong/CatVTON` | **CC BY-NC-SA-4.0** ("code, checkpoints, and demo") | ✅ README | README, licence statement, architecture summary | Concepts only: conditioning on a reference needs no second encoder, only a shared representation space — an argument for one multimodal space rather than per-modality retrieval subsystems | Everything, including the demo. Evaluation additionally touches DeepFashion, VITON-HD and Dress Code |
| VITON-HD + Dress Code | `shadow2496/VITON-HD`, `aimagelab/dress-code` | VITON-HD: **CC BY-NC-4.0**, research only. Dress Code: **proprietary bespoke agreement** — "will not be released to private companies" | ✅ README / ✅ terms | Licence statements and access procedures | Concepts only: the access process is a negative template for our License Guard — "the file downloaded" and "we may use it" are separate facts, which is why `Reference.status` is a four-value enum | No images, no derived features, no checkpoints trained on them. Dress Code requires an institutional email, a hand-signed form, and explicitly excludes private companies |
...
| stable-diffusion-webui-wd14-tagger | `picobyte/stable-diffusion-webui-wd14-tagger` | "Public domain, except borrowed parts" — **non-SPDX, ambiguous** | ✅ README (and found it insufficient) | README | Concepts only: multi-backend interrogation behind one UI with the model as a dropdown — practical proof that backends must never be hardcoded | Any code. Its licence statement names neither an SPDX identifier nor which parts are borrowed under what terms; it would fail our own `license_check`. Reference reading only |
...
| shot-type-classifier | `rsomani95/shot-type-classifier` | **CC-BY-NC-4.0** | ✅ README | README licence section, class list | Concepts only: the single clearest piece of evidence for our alias requirement — three sources, three incompatible conventions for the same ladder | The weights (cannot ship, cannot bundle as an adapter in a commercially capable product) and its class list as our shipped vocabulary. NC is excluded by default under our policy. Cited only |
...
| xoxxel/camera-prompts | `xoxxel/camera-prompts` | MIT (per README licence section; `LICENSE` blob 404'd at the paths tried, so the label is ⚠️ unconfirmed) | ✅ README | README licence section, README structure and term groupings | Concepts only, no data: pairing every vocabulary term with a canonical example image is the cheapest way to make an unfamiliar taxonomy usable — and it works with AI OFF, which v0.1 requires. Our taxonomy nodes carry `visual_hint` for exactly this | Its example PNGs. MIT on a Markdown library does not clear the film frames illustrating it. Our exemplars must come through License Guard from Commons or Openverse under PD/CC0/CC BY only |
| ai-shortfilm-prompts | `jnMetaCode/ai-shortfilm-prompts` | MIT (skill/templates) over **all-rights-reserved third-party prompts** | ✅ README | README, licence statement, structure | Concepts only: a **per-model capability record** — target models differ in duration ceiling, negative-prompt support and filter strictness, so a formatter mode needs a capability record, not just a string template (§13) | Its templates and embedded prompts. This is the perfect trap our review exists to catch: a permissive wrapper around all-rights-reserved content, where an automated "MIT therefore safe" check is simply wrong. Also refused: its advice to specify real lens nomenclature, and one-click genre template packs |
...
| Wikimedia Commons — Action API + CommonsMetadata | `wikimedia/mediawiki`, `.../CommonsMetadata` — `commons.wikimedia.org/w/api.php` | **GPL-2.0-or-later** | ✅ file (CommonsMetadata `COPYING`); ⚠️ UNVERIFIED (MediaWiki core `COPYING` not read — §12) | `COPYING`, `TemplateParser.php`, `DataCollector.php`, `ApiQueryImageInfo.php`, CORS handling, media-type constants | Concepts only, no code: **`AttributionRequired` as an explicit boolean** is the single most reusable idea in the cluster — the source *states* the obligation rather than making us infer it from a licence string, which is precisely `metadata.requires_attribution`. Also: server-side thumbnailing makes `thumbnail_url` a derived URL and never a stored byte | **Any PHP.** GPL-2.0-or-later is copyleft, and our rule is absolute regardless of how the propagation question resolves: no MediaWiki or CommonsMetadata source is vendored, imported or translated, and the provider talks to `api.php` over HTTP only. Whether an HTTP call across a process boundary constitutes a derivative work is `(UNVERIFIED — legal question)`. Also refused: scraping category trees into `data/taxonomy/` (our vocabulary is our own), re-hosting media, and importing free-text descriptions as if they were structured intent |
...
```
   ...
   │                                                        used in practice: 0
   ├── copyleft (GPL-3.0, AGPL-3.0, EUPL-1.2, ............. code never touched;
   │             GPL-2.0-or-later)                          Commons is called over HTTP
...
```
   ...
   ─────────────                    ─────────                          ────────────────
   framing.medium_long_shot     ◄── "MLS", "medium wide shot",    ──►  per formatter_mode
   (ours, stable, lowercase,        "cowboy shot", "american shot"     via model_hints
...
|---|---|---|---|---|
| Qwen3.5 small family (analysis) | ⚠️ reported Apache-2.0; not read firsthand | ⚠️ UNVERIFIED per checkpoint | Unstated | **No.** A reported licence is not a read one (V1). Recorded because the brief names "Qwen3.5-9B class for analysis" as a candidate to *evaluate, not hardcode*; the 2B/4B Qwen3-VL tiers are the realistic laptop-local floor and supersede it in practice ([`research/video-analysis-and-local-runtimes.md`](./research/video-analysis-and-local-runtimes.md)) |
| Qwen3-VL (analysis) | Apache-2.0 ✅ label | ⚠️ UNVERIFIED per checkpoint | Unstated | **No.** Weights unread. Opt-in adapter only |
| Qwen3-VL-Embedding / Reranker | Apache-2.0 ✅ file | ⚠️ UNVERIFIED | Unstated | **No**, same reason. This is the brief's own named candidate — and both tiers it names, the ~2B retrieval tier and the 8B "enhanced search mode" tier, share these same four licence layers — so neither gets defaulted on an unread licence |
...
| LTX-Video | Apache-2.0 ✅ README | **OpenRAIL-M** ✅ | Unstated | **No.** OpenRAIL-M carries use restrictions; it is a formatter *target*, not a bundled component |
| Wan 2.1 / 2.2 | Apache-2.0 ✅ README | Not separately stated | Curated aesthetic data | Formatter target only. Never bundled |
...
| IDM-VTON / OOTDiffusion / CatVTON | CC BY-NC-SA-4.0 (**code and checkpoints**) ✅ file (OOTDiffusion) / ✅ README (IDM-VTON, CatVTON) | Same | VITON-HD / Dress Code | **No, categorically.** Nothing shippable at any layer |
...
| 12 | moondream's **weights**, distinct from the Apache-2.0 code repository | §9.2 records it as a strong candidate for local-first analysis; no recommendation until the checkpoint terms are read |
| 13 | Qwen3-VL's per-checkpoint weight terms, distinct from the repository, whose own `LICENSE` is so far label-only evidence | Recommending any Qwen3-VL tier as the analyzer adapter |

**Blocking a data or taxonomy decision:**

| # | Item | Gates |
|---|---|---|
| 14 | Fashionpedia's dataset and ontology licence, from the primary terms page | Whether it may even be *cited* in `data/taxonomy/clothing.json` |
| 15 | Fashion-IQ's CDLA variant — permissive vs share-alike | Any reference to its material |
| 16 | CineScale / CineScale2 licence, from the dataset records | Citing the angle ⊥ level finding as sourced rather than as our own reasoning |
| 17 | CameraBench's full primitive list, unavailable in the repository | Finalising the motion taxonomy |

**Blocking a provider (v0.2):**

| # | Item | Gates |
|---|---|---|
| 18 | Commons search parameters for user-driven search, and whether its media-search ranking is API-reachable | `src/providers/wikimedia.js` |
| 19 | The `filetype:` alias spelling on Commons | Reliable video filtering |
| 20 | MediaWiki **core**'s own `COPYING`, which the research session never read — the GPL-2.0-or-later term in §4.7 rests on the CommonsMetadata `COPYING` plus core's stated policy | Nothing today: the provider is HTTP-only and vendors no PHP (§4.7). Recorded because the row carries a `⚠️ UNVERIFIED` mark. The file is reachable at `raw.githubusercontent.com/wikimedia/mediawiki/master/COPYING` |
| 21 | Openverse anonymous CORS behaviour behind its CDN, and whether the anonymous cap is per-IP or per-origin | Whether Openverse is default or opt-in, and whether users must register credentials (an egress event to disclose) |
| 22 | Openverse's exact attribution string template | Whether our attribution builder defers to it or derives its own |
| 23 | Whether the Smithsonian API still serves traffic post-archival, and whether the NYPL Repo API is dead | Adding either as a provider |
| 24 | Whether any CORS-enabled, key-free, CC-licensed **video** source exists besides Commons | Whether v0.4's corpus is user-uploaded video only — which the UI must then say |
| 25 | Whether reading pixels from a cross-origin provider video is possible under their CORS headers | Whether remote video can be decomposed at all, or only local uploads |
| 26 | Whether the optional CC BY-SA tier creates a share-alike obligation on a generated *prompt* — a text derivative of attribute descriptions | `(UNVERIFIED — legal question)`. Enabling CC BY-SA anywhere by default |
| 27 | Rijksmuseum Data Services' per-object licence statement, which no readable licence file covers | Adding it as a provider at all — "museum" does not mean "public domain" |

**Blocking a specific feature:**

| # | Item | Gates |
|---|---|---|
| 28 | One vendor's documented pan/tilt axis convention, which is inverted relative to standard cinematography and to a competitor's own commands | That vendor's formatter mode, which is wrong half the time until an empirical A/B settles it |
| 29 | Which video models distinguish dolly-in from zoom-in in *rendered behaviour*, not merely vocabulary | Whether the distinction must survive every formatter mode or only some |
| 30 | Exact geometric-estimator API contracts (docs host blocked) | Writing the ALG-4 specification |
...
| 2 | **Verification marks** `✅ file` / `✅ README` / `✅ label` / `✅ absence` / `⚠️ UNVERIFIED` / `⛔ none` / `§ proprietary` | §2.4, §4 | This document's own evidence grading. Not a schema enum. Deliberately *not* reusing `LICENSE_ID` values, which describe media licences, not our confidence in a claim |
