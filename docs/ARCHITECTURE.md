...
1. Unresolved conflicts are surfaced on the node, not silently dropped. In a headless graph the node still emits a prompt — using the same deterministic winner the UI would show — and reports every conflict as a warning carrying the `conflict_id` and the losing value. The doctrine does not weaken because there is no human on screen; it becomes a visible warning rather than an empty slot.
...
| **C8** | **Conflicts must never be auto-resolved or silently dropped.** Detection is graded (PRODUCT_BRIEF A-1): a HARD conflict emits a deterministic, visible winner the user can flip in one click and keeps the loser in the mix; a SOFT conflict emits both with a warning. Generation is never blocked. | INV-MIX-2. This is the pillar that separates the product from similar-image search. | schema defaults + tests over `applyMix` and `buildStructuredPrompt` |
