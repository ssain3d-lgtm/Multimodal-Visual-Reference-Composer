...
| ID | Title | Path | Deps | Size |
|---|---|---|---|---|
| T-05 | The seven frozen tables, defined once | `src/core/taxonomy.js`, `tests/unit/taxonomy-tables.test.mjs` | T-01 | S |
...
**T-05** — Exports `VISUAL_CATEGORIES` (20, brief order), `INTENT_CATEGORIES` (19), `PROMPT_SLOTS` (13), `CATEGORY_ARITY` (7 `single_dominant` / 13 `multi`), `CATEGORY_TO_PROMPT_SLOT` (20 → 13), `PROMPT_SLOT_EMIT_ORDER`, `EXTRACT_GROUPS` (8 groups covering 17 categories). A test asserts each table is **deep-equal to its counterpart in `docs/schemas/taxonomy-node.schema.json#/$defs`** — the three list tables mirror an `enum` (`visual_category`, `intent_category`, `prompt_slot`), the four map tables a `const` (`category_arity_map`, `category_to_prompt_slot_map`, `prompt_slot_emit_order_map`, `extract_group_map`), and `EXTRACT_GROUPS` mirrors both: `extract_group` for the group names, `extract_group_map` for the categories each covers. The schema is the source, this module is the mirror, and drift fails the build. All exports `Object.freeze`d.
...
**T-18** — `arity`: a `single_dominant` category with ≥ 2 distinct **non-modifier** values from ≥ 2 distinct sources. `exclusivity_group`: a `multi` category with ≥ 2 values sharing a non-null group (`group` required on the record). `explicit`: `conflicts_with`, symmetric, may cross categories. Three negative tests are mandatory and named in the suite: the same value from two references is **agreement**; a modifier in a single-dominant category (`weather.fog`, `camera_angle.dutch_tilt`, `pose.arms_crossed`) never conflicts; two different categories **with no `conflicts_with` edge between their values** never conflict — the brief's second success case (`motion` from video A, `camera_motion` from video B) produces zero conflicts. The qualifier is load-bearing: the shipped taxonomy declares cross-category `conflicts_with` edges (`camera_angle.front` ⇄ `framing.point_of_view`), and an unqualified test would fail on them the day one is exercised. `dominance` is consulted **first** and suppresses the open conflict. Nothing is deleted and nothing is auto-picked (`DOD-01-5`).
...
**T-20** — While `auto_resolve === false`, `resolveConflict` accepts exactly two strategies — `"user"` and `"keep_both"` — and refuses `priority`/`weight`/`first`/`last`, which need the explicit automation opt-in. Both accepted strategies are human decisions: the third button on the conflict card ([`UNIFIED_MODAL_STATE.md`](./UNIFIED_MODAL_STATE.md) §7.5) dispatches `keep_both`, so a rule admitting only `"user"` would make a brief-mandated button undispatchable. `winner_value` is required unless `strategy === "keep_both"` (INV-MIX-3). A `"user"` resolution materialises a `mix_resolution` chip with `ref_id` = winner and **leaves the losing chip in `intent`, marked unused**; `keep_both` moves the conflict to `ignored` with `disposition:"keep_all"` and both values keep emitting. `ignoreConflict(mix, conflict_id, actor)` is the direct-call spelling of that same decision — one mechanism, two entry points, and a test asserts the two produce byte-identical state. `setDominance` resolves and remembers.
...
**T-27** — `SAVE_RECIPE` freezes a `ReferenceSnapshot` for **every** `mix.references[].reference_id` (INV-RCP-1), builds `license_summary` with `attribution_block` from stored text, and marks the session clean. `LOAD_RECIPE` **re-derives** the prompt and treats `prompt_preview` as advisory, emitting a "regenerated" notice on difference; deprecated ids are rewritten via `replaced_by` or kept as custom chips badged "deprecated" — never dropped. A recipe whose snapshots are all `media_state:"gone"` produces the **identical prompt text** (R2 of the media-rot policy). The three remaining events in this task: `SET_KEEP_CHANGE` (22) validates `keep ∩ change === ∅` before any write (INV-EXP-5) and, under D-4, **refuses a non-empty `change[]` whole with a reason** rather than storing it and ignoring it — an accepted-but-ignored CHANGE axis returns the wrong result set while looking like it worked; `difference.enabled` becomes `true` only when `keep` is non-empty. `COMPOSE` (23) recomputes `prompt_preview` from `sp = buildStructuredPrompt(intent, taxonomy, mix)` followed by `formatPrompt(sp.prompt, prompt_mode, { blocked: sp.blocked })` — the **`.prompt` is load-bearing**: `buildStructuredPrompt` returns the `{prompt, blocked}` wrapper while `formatPrompt`'s first parameter is the bare 13-slot `StructuredPrompt` ([`ARCHITECTURE.md`](./ARCHITECTURE.md) §2.3), and passing the wrapper fails `structured-prompt.schema.json` on the first required slot. `blocked[]` reaches the formatter result through `options` (flagged in §9.1) rather than being recomputed there. `COMPOSE` sets `prompt_mode` if the payload carries one, and touches nothing else. `LOAD_PRESET` (27) merges the preset's chips with `source:"preset"` and leaves every `user` and `locked` chip untouched; the file it reads is the `data/presets.json` T-67 authors.
...
| T-32 | `filterReferences` — the hard gate | `src/search/metadata-search.js`, `tests/unit/search-filters.test.mjs` | T-29, T-41, T-65 | M |
| T-33 | `searchReferences` — targets, `REL` credit, `metadata_match` | `src/search/metadata-search.js`, `tests/unit/search-metadata.test.mjs` | T-32, T-65 | L |
...
**T-31** — Three edge kinds, exactly per [`SEARCH_ARCHITECTURE.md`](./SEARCH_ARCHITECTURE.md) §8.1: symmetric `related[]` at `RELATED_HOP × score(source)`, `children[]` of a hit **branch** node at `CHILD_HOP × score(source)` — both frozen in T-30's `KEYWORD_SCORES` at 0.35 and 0.30 — and `deprecated → replaced_by`, which is a rewrite at 1.00 performed in `normalizeVisualIntent`, not an expansion. Without the child hop, hitting a branch node is a dead end. Controls, all frozen: seeds = the top 5 node hits by `score_kw`; hops = 1, never 2; cap = 24 expansion nodes, then truncated by score; an expansion hit that is also a direct hit keeps the max, so the direct hit wins. The expanded node's own `search_boost` multiplies the hop score **last**, as §8.2's arithmetic does (`0.35 × 0.5175 × 1.10 = 0.199`). Works with `ai.enabled === false` — expansion needs no model, and a test asserts it runs with the NULL analyzer and NULL embedding registered. Expanded hits become `source:"query_expansion"` chips, low confidence, one click to remove.

**T-32** — Filters **exclude**, they never down-rank. Defaults `status:["approved"]` and the allowed-by-default licence set. An empty pool is a **completed** retrieval, so the `ResultSet` still carries `status:"ready"` with `total: 0` and `items: []` — there is no `empty` status, because the canonical enum is `idle | loading | ready | error` ([`schemas/explorer-state.schema.json`](./schemas/explorer-state.schema.json)). What makes the case explicable is the per-filter exclusion tally `filterReferences` returns alongside the surviving set, which the UI states with counts (`"0 of 340 matches are CC BY or freer — 218 are excluded by licence."`, [`SEARCH_ARCHITECTURE.md`](./SEARCH_ARCHITECTURE.md) §4.1, INV-SRCH-13). Silently widening the query is forbidden and a test asserts the filter set in the result equals the filter set in the query.

**T-33** — `REL` frozen at IDENTICAL 1.00 / DESCENDANT 0.60 / ANCESTOR 0.45 / SIBLING 0.30 / RELATED 0.25 / NONE 0.00, with the `0.75^(d-1)` decay and `d ≤ 3`. `metadata_match` normalised 0..1 by construction. Category weights default uniform. Golden test: the four-reference worked example in §4.4 reproduces `1.000 / 0.565 / 0.177 / 0.000` — the values that table derives from these same `REL` constants (`img_b` = (0.30+1.00+0.54×0.25)/2.54, `img_c` = 0.45/2.54); a golden file that disagrees with the frozen table is a bent scorer, not a bent fixture. Every `ScoredRef` also carries the explainability payload the schema already declares — `matched_categories` and `matched_values`, the categories and the stored taxonomy ids that actually earned credit, partial credit included with the `REL` relation that earned it — because the scorer is the only place that relation is known and [`SEARCH_ARCHITECTURE.md`](./SEARCH_ARCHITECTURE.md) §10 makes the explanation a product requirement, not a nicety.
...
**T-35** — With no embedding adapter, `ranking.mode` falls back to `metadata_only` / `keyword_only` and the results header **shows it**, so nobody believes semantic search silently ran (INV-AI-1). `fuse()` implements min-max weighted sum with defaults 0.6 / 0.4 for the day `v0.3` arrives; in v0.1 the semantic list is empty and fusion degenerates to the metadata list, which the test asserts explicitly. `semantic-search.js` ships as the index + `knn` skeleton with the NULL adapter path only. `buildResultSet` fills each item's `score_breakdown` with the **normalized, pre-weight** values and uses `null` — never `0` — for a signal that did not exist for that item, which in v0.1 is `semantic` and `rerank` on every item ([`SEARCH_ARCHITECTURE.md`](./SEARCH_ARCHITECTURE.md) §10.1); the two render differently downstream. `explain(query, reference, ranking)` ships here too (§10.2, flagged in §9.1): pure, reproducible from stored state (INV-SRCH-6), covering the signals v0.1 has — metadata, keyword, the KEEP axis — and reporting `semantic`/`rerank` as unavailable rather than as zero. Its test asserts `Σ contribution_pct === 100` and that the reconstructed score equals `items[i].score` to 1e-9.
...
| T-40 | Golden conformance fixtures + determinism | `tests/conformance/*.json`, `tests/conformance/prompt.test.mjs` | T-39, T-65 | M |

**T-36** — Routing is table-driven from `CATEGORY_TO_PROMPT_SLOT`; a chip with `negate:true` routes to `constraints` **regardless of category**, emitting `negative_fragment ?? "no " + fragment` with `kind:"negative"`. Fragment text is **mode-neutral** — `prompt_fragment ?? label ?? humanize(value)`, with no mode in scope ([`DATA_SCHEMA.md`](./DATA_SCHEMA.md) §12.3 step 3); `model_hints[mode]` is resolved later, by `formatPrompt` (T-38), which is what keeps a built `StructuredPrompt` portable across modes and INV-FMT-1 a statement about the formatter rather than a tautology. Custom chips emit their label verbatim. Every `PromptFragment` carries `{source_category, value, ref_id, chip_id}` — provenance is not optional (`DOD-01-7`). Every fragment in the `lens` slot has `hedged: true` (INV-LENS-2).
...
**T-38** — `registerFormatter(mode, fn)`; an unknown mode falls back to `generic` **with a warning** rather than throwing (`formatter_mode` is an open string by design). This is the one place a mode enters: `formatPrompt` re-resolves every fragment through `Taxonomy.fragment(fragment.value, mode)` (`model_hints[mode] ?? prompt_fragment ?? label ?? humanize`, [`DATA_SCHEMA.md`](./DATA_SCHEMA.md) §12.3) — a mode that defines no hint leaves the built text untouched, a `custom` fragment has no taxonomy `value` and is emitted verbatim in every mode, and a `model_hints` override in the `lens` slot must itself carry the hedge (INV-LENS-2). `formatPrompt` is pure: same `StructuredPrompt` + mode + taxonomy version ⇒ byte-identical text (INV-FMT-1). It never invents a value absent from the `StructuredPrompt` (INV-FMT-3) — asserted by comparing the emitted token set against the fragment set. Still-image modes drop `camera_motion` fragments with a warning (INV-FMT-2).
...
**T-50** — The mock returns a **fixed** intent, never derived from the uploaded bytes, and it drives `analysis_status: "mocked"`. Two loading paths, deliberately separate. The **shipped** one is a single small ingest per modality held inline in `src/ai/analyzer.js` — taxonomy ids and `evidence` only, labelled as the demo ingest ([`ARCHITECTURE.md`](./ARCHITECTURE.md) §5.2) — selected by the upload's modality and by nothing else, so an image upload always yields the image ingest and a video upload the video ingest; there is no hashing, no round-robin and no inspection of the file. The **test** corpus of richer per-case ingests lives under `tests/fixtures/mock-analyzer/` and is injected by the test (D-7); `app/index.html` fetches nothing from `tests/` (T-04), which is why the shipped path cannot be a fixture file. The shipped image ingest must contain `composition.rule_of_thirds` (QA-09) and the shipped video ingest a `camera_motion` value carrying `evidence.t_start_s` / `evidence.t_end_s` (QA-18), or two of the four input modes are decorative and the P2 row of §8.1 is unprovable. Façade post-conditions: a `camera_motion` chip sourced `analyzer_image` is **dropped with a warning** (INV-VID-1); a `motion` chip from `analyzer_image` is clamped to `evidence.kind:"implied"`, `confidence ≤ 0.6`, and kept only for nodes flagged `still_inferable` (INV-VID-3). A third-party adapter therefore cannot corrupt the product's epistemic stance.
...
**T-52** — `mountExplorerModal(el, deps) -> {dispatch, getState, subscribe, destroy}`; `deps` carries `{dispatch, taxonomy, capabilities}` and **never a network client**. Effects run **after** the state is committed and rendered, so the user always sees `loading`/`pending` before any I/O. Escape is layered (§9.4 of the modal doc) and never destroys work; closing a dirty session requires explicit confirmation, never Escape alone. Dialog semantics, focus trap, and `aria` roles per §9.1. The runner owns a handler for **every** one of the eight effect kinds T-23 freezes; in v0.1 `analyze` reaches the mock analyzer, `provider_call` reaches `local.js` only, `disclose` opens R12, and **`persist` writes the session and the saved `VisualRecipe` to `localStorage` under a named key** ([`UNIFIED_MODAL_STATE.md`](./UNIFIED_MODAL_STATE.md) §4, "`persist` → local storage"; the quota failure copy is §10.3) — this is the medium QA-23's page reload survives, and the recipe *file* of §7 is the export path of T-62, not the reload path. `persist` is fire-and-forget and never blocks a render. The shell owns the **read** half as well: on mount it rehydrates from the same namespaced, versioned keys — one session envelope plus a recipe index keyed by `rcp_` id, which is what gives event 28 (`LOAD_RECIPE {id}`) an `id` to find after a reload — and an absent, unparseable or wrong-version blob starts a clean session with a non-blocking notice, never a thrown error and never a silent overwrite of what is stored. A write that exceeds the quota is the toast of [`UNIFIED_MODAL_STATE.md`](./UNIFIED_MODAL_STATE.md) §10.3 (`Session not saved: storage full.`) with the in-memory session untouched. All of this lives in the shell: rule 2 keeps `localStorage` out of `src/core/**`, so the reducer only ever emits the effect. The shell also renders the history chrome [`UNIFIED_MODAL_STATE.md`](./UNIFIED_MODAL_STATE.md) §5.5 scopes to v0.1: the `←` back affordance bound to `NAVIGATE_BACK` and `Alt+←`, the **read-only** breadcrumb list, and **no forward button**; plus the §9.3 global shortcut map (`/`, `Ctrl/Cmd+Enter`, `Alt+1..4`, `Ctrl/Cmd+Shift+C`, `?`) and the `?` shortcut sheet that lists them. A review checklist item asserts §9.3's binding rule — every shortcut has a pointer-reachable equivalent — since a `history` slice with no rendered control is a tested feature the user cannot reach.
...
**T-58** — A card with no onward action is forbidden. EXTRACT expands the group name to categories **before** dispatch. Repeated EXTRACT is additive. Clicking × on a chip a mix entry contributes writes `exclude: [<taxonomy_id>]` into that entry — **deleting the chip alone would be undone by the next `applyMix` and reads as a haunted UI**; this is the single most important implementation detail on the card. An EXPLORE action whose categories are empty on that reference is disabled with a reason on hover and focus, never silently returning zero results. The card also carries levels 1 and 3 of the explainability requirement ([`SEARCH_ARCHITECTURE.md`](./SEARCH_ARCHITECTURE.md) §10.3): always the `matched_values` chips, exact matches solid and partial ones (descendant / ancestor / sibling / related) outlined, so the user sees *which* of their chips this card answered; and on demand the `explain()` popover of T-35, rendered as the sentence-plus-table of §10.3. Level 2, the ranking readout, is the results header of T-35.
...
**T-62** — 13 slots in `GENERIC_SLOT_ORDER`; every fragment traceable to its chip, category and reference on hover and focus; a blocked slot renders as a **pending decision with a jump link to R8**, never as an error and never as a silent omission. Exports: `prompt`, `structured_prompt`, `visual_intent`, `reference_mix` — the same four the future ComfyUI node emits. Save is paired with **load**: a saved-recipe list, read from the recipe index T-52 rehydrates, dispatches `LOAD_RECIPE {id}` and is the reload half QA-23 walks; the exported recipe *file* is the §7 path and is not the reload path.
...
**T-64** — The ingest tool is **not part of `src/`**, is not loaded by `app/index.html`, and is run by a maintainer **with** network access. It fetches provider metadata, mints deterministic ids, runs all four guard stages including SOURCE VALIDATION, and writes `data/references.json` with the full `license_guard` audit trail. This is how the shipped library can contain `approved` records while the shipped **app** makes no network call: LP-21 constrains the runtime, and the runtime still re-validates and demotes on load (T-46). The tool stores metadata, URLs, thumbnails, `visual_attributes` and attribution text — **never bytes**. `visual_attributes` are **not** provider metadata and no v0.1 code infers them: the tool maps a provider's tags, title and categories through the taxonomy's `aliases[]` into taxonomy ids as a *proposal*, and a maintainer confirms or corrects every reference in a review pass before it is written. An unmapped tag is dropped, never guessed into a neighbouring id. This is the one manual labour item in v0.1 and it is named here rather than discovered during T-66, because Pillars 3, 4 and 5 all read from this field and a thin or wrong library disables all three at once.
...
**T-66** — 300–600 references, spread so **every one of the 8 EXTRACT groups has ≥ 30 examples**. A thinner library invalidates `DOD-01-3`: browse and EXTRACT read as a toy and the demo cannot be completed honestly. Volume is only half the bar; the other half is **density per reference**, because a library of 600 references each carrying two categories satisfies the group spread above and still leaves decomposition, selective inheritance and mixing with nothing to work on. Every SEED-B reference therefore carries non-empty values in **≥ 8 of the 20 categories** and **≥ 4 of the 8 EXTRACT groups**, and the tranche's median is **≥ 14 values per reference** (thresholds chosen here — §9.1). The ingest tool's census reports both distributions alongside the licence census, fails the tranche if either floor is missed, and is pasted into the PR description.

**T-67** — `data/presets.json` with `pst_` ids, each preset a set of chips (never a prompt string). `visual_hint` assignment: every node that appears as a browse tile in **any of the 20 categories** gets a `visual_hint` pointing at a seed reference, plus `visual_hint_pool` where a single image would mislead. The scope is all 20 and not just the `single_dominant` seven because the browse grid exposes all 20 ([`UNIFIED_MODAL_STATE.md`](./UNIFIED_MODAL_STATE.md) §10.1) and because *"I don't know what this lighting is called"* is one of the brief's named success cases while `lighting` is a `multi` category — scoping the pass to `single_dominant` would answer the low-angle case and drop the lighting one. Validator extension: a browse-reachable node without a `visual_hint` is a warning and the count is reported per category; the count for `camera_angle` and `lighting` must be **zero**, since those two carry the brief's two "pick it by looking" cases (QA-05).
...
| D-7 | The mock analyzer's **test** fixtures live under `tests/fixtures/`, never `src/ai/`; the one shipped demo ingest is inline in `src/ai/analyzer.js` and labelled as such | A fixture *corpus* inside the shipping source tree becomes the de-facto spec for the real analyzer within one milestone. But `app/index.html` fetches nothing from `tests/` (T-04), so a mock with no in-tree ingest leaves IMAGE and VIDEO decorative in the only milestone that ships them |
...
Waves are dependency-derived, not calendar-derived. Nothing inside a wave depends on a later wave — but a wave is not a flat batch: several hold internal chains (`T-10 → T-11 → T-12 → T-13`, `T-44 → T-45 → T-46`, `T-17 → T-18 → T-19 → T-20`, `T-10 → T-49 → T-50`), so within a wave the Deps column still orders the work.

```
WAVE 1  scaffold + tables            T-01 T-02 T-03 T-04 T-05
   │
WAVE 2  core vocabulary              T-06 T-07 T-08 T-09 T-41
   │
WAVE 3  core semantics               T-10 T-11 T-12 T-13 T-14 T-15 T-16 T-42 T-43
   │                                 T-44 T-45 T-46 T-47 T-48 T-64 T-65 T-49 T-50
   │
...
**Critical path:** `T-01 → T-05 → T-06 → T-07 → T-09 → T-10 → T-11 → T-12 → T-17 → T-18 → T-19 → T-20 → T-25 → T-58 → T-60 → T-61 → T-62 → T-71`. Every arrow is an edge in the Deps column; a path that skips one is a schedule that cannot be executed. It runs through the mix and the conflict UI, which is deliberate: **the conflict UI is the hardest surface in the product and it lands in the first milestone**, because discovering that it is hard at v0.4 — after three milestones have been built on top of it — would be fatal.
...
| **P2 Visual Intent** | Text, browse, mocked image and mocked video all produce chips in the **same** 20-key document | QA-02, QA-05, QA-09, QA-18 | T-10, T-24, T-50 |
...
| **P3 Reference Decomposition** | A card exposes all 20 categories with per-attribute confidence; 8 EXTRACT groups reach 17 of them; `subject`/`appearance`/`action` are read-only with a stated reason | QA-10 | T-58, T-59 |
| **P3** | No card exists without an onward action | code review + T-58 test | T-57, T-58 |
| **P4 Selective Inheritance** | Composition from A and clothing from B produce one intent with per-chip `ref_id` provenance | QA-11, QA-12 | T-17, T-25, T-58 |
...
| **P5 Reference Mixing** | Two references contributing different values in one `single_dominant` category produce **one open conflict**, both values remain in `intent`, the slot is `blocked`, nothing is auto-picked | `DOD-01-5` + QA-14, QA-16 | T-18, T-37, T-61 |
| **P5** | The conflict UI names both sources, pre-selects the user's own value, and uses no error vocabulary | QA-15 copy review | T-61 |
| **P5** | A resolution survives recomputation (deterministic `cfl_` id) and is remembered via `dominance` | QA-17 | T-19, T-20 |
...
| **Cross** | "mock" is visible wherever `analysis_status === "mocked"` | `DOD-01-12` + QA-07, QA-18 | T-54 |
...
| QA-02 | T1 | Type `golden hour alley`, submit | All three §3.5 hits enter `intent` as `source:"user"` — `lighting.golden_hour_sun` and `time.golden_hour` (prefix + word-internal prefix, ranked in that order by `search_boost`) and `scene.alley` (exact label) — each carrying the **pre-boost `s_raw`** as its confidence (`0.443 / 0.443 / 0.317`, T-30), never a boosted `score_kw` and never a flat `1.0`; one `query_expansion` chip appears greyed; results header reads `keyword_only` | A model runs; one of the two golden-hour senses is guessed away; a chip's confidence is the boosted score; expansion chips are indistinguishable from user chips |
...
11. **`formatPrompt`'s `options` argument carries `{ blocked?: BlockedSlot[] }`.** [`ARCHITECTURE.md`](./ARCHITECTURE.md) §2.3 names the parameter and declares `blocked[]` on the return value but fixes no shape for the argument; T-27's `COMPOSE` expression assumes this one key and no other. It is a call-shape assumption, not a schema field.
12. **The SEED-B attribute-density floors** in T-66 — ≥ 8 of 20 categories, ≥ 4 of 8 EXTRACT groups, median ≥ 14 values per reference — are chosen here. No other document states a density bar; [`ROADMAP.md`](./ROADMAP.md) §2.1 fixes only the 300–600 budget and the ≥ 30-per-EXTRACT-group spread. They are a floor for a demo library, not a measurement (UNVERIFIED).
13. **`explain()` is scheduled in T-35 although [`ARCHITECTURE.md`](./ARCHITECTURE.md) §2.2's `fusion-ranker.js` block does not list it.** The function, its `Explanation` shape and its test bar are specified in [`SEARCH_ARCHITECTURE.md`](./SEARCH_ARCHITECTURE.md) §10.2, and §10.3 makes all three rendering levels mandatory, so v0.1 builds it rather than deferring a stated product requirement. It is the one addition to the API inventory of §9.2 claim 1.

### 9.2 Claims other documents must agree with

1. **The public API surface implemented by these tasks is exactly [`ARCHITECTURE.md`](./ARCHITECTURE.md) §2**, function for function, with the single addition of `fusion-ranker.js`'s `explain()` (§9.1 item 13). The only source file added beyond that inventory is `src/core/hash.js`.
